...
def trimming_umis(reads_fastq, umis_fastq, out_umis_fastq):
    ...
    for read in SeqIO.parse(reads_fastq, "fastq"):
        reads[str(read.id)] = read 
    #print("Ids de reads obtenidos", '\n')

    #print("Obteniendo Ids de umis", '\n')
    for umi in SeqIO.parse(umis_fastq, "fastq"):
        umis[str(umi.id)] = umi
    #print("Ids de umis obtenidos", '\n')

    shared_ids = reads.keys() & umis.keys()
    #print("shared_ids: ", shared_ids, '\n')


    trimmed_umis = {id_ : umis[id_] for id_ in shared_ids}
    #print("Filtrado de UMIs realizado")

    trimmed_umis_ids = list(trimmed_umis.values())

    #print("Escribiendo fastq de UMIs filtrados", '\n')
    SeqIO.write(trimmed_umis_ids, out_umis_fastq, "fastq")
    return True
